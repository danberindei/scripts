...
def notify(summary, message, icon=None):
    ...
    if icon:
        ...
        l.append("skype")
    l.extend(["-a", "skype", "--hint=int:transient:1", "--hint=string:append:allowed", summary, message])
    # print l # For debugging.
    with open("/tmp/skype-notifications.log", "a") as f:
        f.write("%s\n" % l)
...
notifications = {
	...
	"ContactAuthRequest":
        ("Contact request from \"{sname}\"", "{sname}\n{sskype}", None),
    "ContactDeleted":
